...
const DiscoverEvent = ({ eventState }) => {
  return (
    <TabsContent className="pt-[32px] text-foreground" value="discover">
      {eventState?.loading && (
        <div className="mb-[48px] mt-[64px] flex h-[348px] flex-col items-center justify-center">
          <LuLoader className="h-10 w-10 animate-spin" />
        </div>
      )}
      {eventState?.allEvent?.length === 0 && !eventState?.loading && (
        <div className="mb-[48px] mt-[64px] flex h-[348px] flex-col items-center justify-center">
      ...
      {eventState?.allEvent?.length > 0 && !eventState?.loading && (
        <div className="w-full mx-auto py-4">
          <div className="relative border-l-[2px] border-dashed border-muted-foreground md:ml-[200px]">
            {eventState?.allEvent?.map((event, index: number) => (
